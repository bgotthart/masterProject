 ...
 */
class DBpedia_DatabaseLocalClass extends DatabaseClass {
    ...
    public function __construct($dbpedia_store) {

        $this->dbpedia_store = $dbpedia_store;
    ...
    public function getCategoriesOfCategory($category) {
        ...
        try {
            $q = 'SELECT DISTINCT ?x WHERE { <http://dbpedia.org/resource/Category:' . $category . '> <http://www.w3.org/2004/02/skos/core#broader> ?x . }';
    ...
    private function get3LevelCategories($term, $level) {
        ...
        try {
            if ($level == 1) {
                ...
                $q = 'SELECT DISTINCT * WHERE { <http://dbpedia.org/resource/Category:' . $term . '> <http://www.w3.org/2004/02/skos/core#broader> ?a . 
                     ?a <http://www.w3.org/2004/02/skos/core#broader> ?b  . 
                     ?b <http://www.w3.org/2004/02/skos/core#broader> ?c . 
                     ?c <http://www.w3.org/2004/02/skos/core#broader> ?d 
                    }';
            ...
            if ($rows = $this->dbpedia_store->query($q, 'rows')) {
                ...
                foreach ($rows as $row) {
                    ...
                    if (isset($row['a'])) {
                        array_push($result, $row['a']);
                    }
                    if (isset($row['b'])) {
                        array_push($result, $row['b']);
                    }
                    if (isset($row['c'])) {
                        array_push($result, $row['c']);
                    }
                    if (isset($row['d'])) {
                        array_push($result, $row['d']);
                    }
                }
            }
            return $result;
        ...
        try {
            ...
            if ($rows = $this->dbpedia_store->query($q, 'rows')) {
                ...
                foreach ($rows as $row) {
                    ...
                    array_push($this->main_topic_classification, $row['x']);
                }
            }
        } catch (SQLException $oException) {
            echo ("Caught SQLException: " . $oException->sError );
